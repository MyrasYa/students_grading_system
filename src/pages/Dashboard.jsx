...
function Dashboard() {
  ...
  const passed = students.filter((s) => Number(s.score) >= 75).length;
  const failed = total - passed;


  return (
    <div>
      <h1 className="text-right text-[44px] antialiased font-bold text-[#4f4a4af6] mb-[-25px] italic">
        Students Grading System
      </h1>

      {/* Input & Statistic Students Section */}
      <div>
        <h2 className="font-bold text-xl text-[#191C1E] mb-1">
          Add New Student
        </h2>
      ...
      <div className="mt-7 rounded-3xl p-4 shadow-sm overflow-hidden bg-[#CDCDCD]/3">
        ...
        <StudentTable students={students} onChange={fetchData} />
      </div>
      {/* Tabel Students Section */}
    </div>
  );
